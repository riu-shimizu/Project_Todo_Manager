...
* **Todo**: 最下層。`status`, `assigneeId`, `dueDate`, `todayFlag` を持つ。Task 配下で並び替え可能。`todayFlag` は列として存在するが、現状の「今日の Todo」抽出は `dueDate = 今日` のみを利用。
...
| `listTodayTodos` | **期限日が今日の Todo** をフィルタ。|
...
| `POST /api/projects` / `PATCH /api/projects/:id` / `DELETE /api/projects/:id` | プロジェクト作成・更新・削除。
...
| `HierarchyView` | Phase/Work/Task/Todo を縦階層で表示。インライン編集、ステータス切り替え、折り畳み表示、追加・削除ボタンを提供。|
